...
def get_matches(image1, image2) -> typing.Tuple[
    ...
    # Create dictionaries to map query indices to train indices
    matches_dict_2_to_1 = {m.queryIdx: m.trainIdx for m in good_matches_2_to_1}

    # Perform left-right check
    matches = []
    for m in good_matches_1_to_2:
        idx1 = m.queryIdx
        idx2 = m.trainIdx
        # Check if there's a match from idx2 back to idx1
        if idx2 in matches_dict_2_to_1:
            if matches_dict_2_to_1[idx2] == idx1:
                matches.append(m)
...
def triangulation(
    ...
    points1 = np.array([kp1[m.queryIdx].pt for m in matches])  # Shape (N, 2)
    points2 = np.array([kp2[m.trainIdx].pt for m in matches])  # Shape (N, 2)

    # Compute projection matrices for both cameras
    # Camera projection matrix: P = K * [R | t]
    # Need to concatenate rotation and translation
    proj_mat1 = camera_matrix @ np.hstack((camera1_rotation_matrix, camera1_translation_vector))
    proj_mat2 = camera_matrix @ np.hstack((camera2_rotation_matrix, camera2_translation_vector))

    # Triangulate points using OpenCV function
    points_4d_hom = cv2.triangulatePoints(proj_mat1, proj_mat2, points1.T, points2.T)  # Shape (4, N)

    # Convert from homogeneous coordinates to 3D points
...
def resection(
    ...
    _, kp2, found_matches = get_matches(image1, image2)
    ...
    for match in found_matches:
        ...
        if query_idx in point_map:
            object_points_list.append(point_map[query_idx])
            image_points_list.append(kp2[match.trainIdx].pt)
    ...
    success, rotation_vec, translation_vec, _ = cv2.solvePnPRansac(
        object_points, image_points, camera_matrix, None
    )

    if not success:
        raise ValueError("no PnPRansac")

    return cv2.Rodrigues(rotation_vec)[0], translation_vec


def convert_to_world_frame(translation_vector, rotation_matrix):
    camera_orientation = rotation_matrix.T
    camera_position = -camera_orientation @ translation_vector.reshape(3, 1)
